...
# === Load dữ liệu ===
df = pd.read_excel("grading_results_rag_metrics.xlsx")  # hoặc pd.read_csv("scores.csv")
...
print(f"MAE (RAG_TRAINING): {mae_RAG:.3f}") # Nếu MAE/RMSE của RAG thấp hơn → RAG tốt hơn.
print(f"MSE (RAG_TRAINING): {mse_RAG:.3f}") # Nếu MSE của RAG thấp hơn → RAG tốt hơn.
print(f"RMSE (RAG_TRAINING): {rmse_RAG:.3f}") # Nếu MAE/RMSE của RAG thấp hơn → RAG tốt hơn.
print(f"QWK (RAG_TRAINING): {qwk_RAG:.3f}") # QWK: giá trị từ -1 đến 1, càng gần 1 càng tốt